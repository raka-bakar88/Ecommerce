...
public class ProductServiceImpl implements ProductService {
    ...
    public ProductDTO addProduct(ProductDTO productDTO, Long categoryId) {
        ...
        // to check if the name is already existing
        ...
        if (isProductNotPresent) {
            ...
            double specialPrice = getSpecialPrice(product.getPrice(), product.getDiscount());
            product.setSpecialPrice(specialPrice);

            Product result = productRepository.save(product);
            return modelMapper.map(result, ProductDTO.class);
        } else {
            throw new APIException("Product with the name " + productDTO.getProductName() + " has already exist");
        }
    }
...
//            throw new APIException("Product data is empty");

        List<ProductDTO> productDTOS = savedProducts.stream().map(product -> modelMapper.map(product, ProductDTO.class))
                .toList();
    ...
    public ProductResponse searchByCategory(Long categoryId,
        ...
        List<ProductDTO> productDTOS = productPage.stream().map(product -> modelMapper.map(product, ProductDTO.class))
                .toList();
